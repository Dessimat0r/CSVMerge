 ...
 */
public class Path implements IPath, Cloneable, Serializable {
	...
	private String[] s_indices;
	private String[] s_specs;
	
	Pattern segmentPattern = Pattern.compile("^([A-Za-z_0-9]*?)(?:<([A-Za-z_0-9]*?)>)?(?:\\[([A-Za-z_0-9*]*?)\\])?");
	 ...
	 */
	public IPath construct(String fullPath) {
		String devicePart = null;
		if (WINDOWS) {
		...
		return initialize(devicePart, fullPath);		
	}
	 ...
	 * Private constructor.
	 */
	private Path(String device, String[] segments, String[] s_names, String[] s_specs, String[] s_indices, int _separators) {
		// no segment validations are done for performance reasons	
		this.segments = segments;
		this.s_names = s_names;
		this.s_indices = s_indices;
		this.s_specs = s_specs;
		this.device = device;
	...
	public IPath addTrailingSeparator() {
		...
		if (isEmpty()) {
			return new Path(device, segments, s_names, s_specs, s_indices, HAS_LEADING);
		}
		return new Path(device, segments, s_names, s_specs, s_indices, separators | HAS_TRAILING);
	}
	...
	public IPath append(IPath tail) {
		...
		String[] newNames = new String[myLen + tailLen];
		System.arraycopy(s_names, 0, newNames, 0, myLen);
		for (int i = 0; i < tailLen; i++) {
			newNames[myLen + i] = tail.name(i);
		}
		
		String[] newSpecialisations = new String[myLen + tailLen];
		System.arraycopy(s_specs, 0, newSpecialisations, 0, myLen);
		for (int i = 0; i < tailLen; i++) {
			newSpecialisations[myLen + i] = tail.index(i);
		}	
		
		String[] newIndices = new String[myLen + tailLen];
		System.arraycopy(s_indices, 0, newIndices, 0, myLen);
		for (int i = 0; i < tailLen; i++) {
			newIndices[myLen + i] = tail.index(i);
		}		
		
		//use my leading separators and the tail's trailing separator
		Path result = new Path(device, newSegments, newNames, newSpecialisations, newIndices, (separators & (HAS_LEADING | IS_UNC)) | (tail.hasTrailingSeparator() ? HAS_TRAILING : 0));
	...
	public IPath append(String tail) {
		...
		if (tail.indexOf(SEPARATOR) == -1 && tail.indexOf("\\") == -1 && tail.indexOf(DEVICE_SEPARATOR) == -1) { //$NON-NLS-1$
			...
			Matcher m = segmentPattern.matcher(tail);
			...
			if (matches) {
				name = m.group(1);
				specialisation = m.group(2);
				index = m.group(3);
			} else {
				name = tail;
			}
			
			String[] newNames = new String[myLen + 1];
			System.arraycopy(s_names, 0, newNames, 0, myLen);
			newNames[myLen] = tail;
			
			String[] newSpecs = new String[myLen + 1];
			System.arraycopy(s_specs, 0, newSpecs, 0, myLen);
			newSpecs[myLen] = tail;
			
			String[] newIndices = new String[myLen + 1];
			System.arraycopy(s_indices, 0, newIndices, 0, myLen);
			newIndices[myLen] = tail;			
			
			return new Path(device, newSegments, newNames, newSpecs, newIndices, separators & ~HAS_TRAILING);
		}
	 ...
	 */
	private void computeSegments(String path) {
		// performance sensitive --- avoid creating garbage
		int segmentCount = computeSegmentCount(path);
		if (segmentCount == 0) {
			segments = NO_SEGMENTS;
			s_names = NO_SEGMENTS;
			s_indices = NO_SEGMENTS;
			return;
		}
		String[] newSegments = new String[segmentCount];
		String[] newNames = new String[segmentCount];
		String[] newSpecs = new String[segmentCount];
		String[] newIndices = new String[segmentCount];
		...
		for (int i = 0; i < segmentCount; i++) {
			...
			Matcher m = segmentPattern.matcher(newSegments[i]);
			boolean matches = m.matches();
	...
	public IPath makeAbsolute() {
		...
		Path result = new Path(device, segments, s_names, s_specs, s_indices, separators | HAS_LEADING);
	...
	public IPath makeRelativeTo(IPath base) {
		...
		String[] newNames = new String[newSegmentLength];
		String[] newSpecs = new String[newSegmentLength];
		String[] newIndices = new String[newSegmentLength];
		...
		System.arraycopy(s_names, commonLength, newNames, differenceLength, newSegmentLength - differenceLength);
		System.arraycopy(s_specs, commonLength, newSpecs, differenceLength, newSegmentLength - differenceLength);
		System.arraycopy(s_indices, commonLength, newIndices, differenceLength, newSegmentLength - differenceLength);
		
		return new Path(null, newSegments, newNames, newSpecs, newIndices, separators & HAS_TRAILING);
	}
	...
	public IPath makeUNC(boolean toUNC) {
		...
		return new Path(toUNC ? null : device, segments, s_names, s_specs, s_indices, newSeparators);
	}
	...
	public IPath removeFirstSegments(int count) {
		...
		if (count >= segments.length) {
			return new Path(device, NO_SEGMENTS, NO_SEGMENTS, NO_SEGMENTS, NO_SEGMENTS, 0);
		}
		...
		System.arraycopy(this.s_names, count, newNames, 0, newSize);
		
		String[] newSpecs = new String[newSize];
		System.arraycopy(this.s_specs, count, newSpecs, 0, newSize);
		
		String[] newIndices = new String[newSize];
		System.arraycopy(this.s_indices, count, newIndices, 0, newSize);
	...
	public IPath removeLastSegments(int count) {
		...
		if (count >= segments.length) {
			...
			return new Path(device, NO_SEGMENTS, NO_SEGMENTS, NO_SEGMENTS, NO_SEGMENTS, separators & (HAS_LEADING | IS_UNC));
		}
		Assert.isLegal(count > 0);
		int newSize = segments.length - count;
		...
		String[] newNames = new String[newSize];
		System.arraycopy(this.s_names, 0, newNames, 0, newSize);
		
		String[] newSpecs = new String[newSize];
		System.arraycopy(this.s_specs, 0, newSpecs, 0, newSize);
		
		String[] newIndices = new String[newSize];
		System.arraycopy(this.s_indices, 0, newIndices, 0, newSize);		
		
		return new Path(device, newSegments, newNames, newSpecs, newIndices, separators);
	}
	...
	public IPath removeTrailingSeparator() {
		...
		return new Path(device, segments, s_names, s_specs, s_indices, separators & (HAS_LEADING | IS_UNC));
	}
	...
	/* (Intentionally not included in javadoc)
	 * @see IPath#specialisation(int)
	 */
	public String specialisation(int index) {
		if (index >= segments.length)
			return null;
	 ...
	 */
	public String[] names() {
		String[] namesCopy = new String[segments.length];
		System.arraycopy(s_names, 0, namesCopy, 0, segments.length);
		return namesCopy;
	}	
	...
	public String[] indices() {
		String[] indicesCopy = new String[segments.length];
		System.arraycopy(s_indices, 0, indicesCopy, 0, segments.length);
		return indicesCopy;
	}
	...
	public IPath setDevice(String value) {
		...
		return new Path(value, segments, s_names, s_specs, s_indices, separators);
	}
	...
	public IPath uptoSegment(int count) {
		if (count == 0)
			return new Path(device, NO_SEGMENTS, s_names, s_specs, s_indices, separators & (HAS_LEADING | IS_UNC));
		if (count >= segments.length)
			return this;
		...
		System.arraycopy(s_names, 0, newNames, 0, count);
		
		String[] newSpecs = new String[count];
		System.arraycopy(s_specs, 0, newSpecs, 0, count);
		
		String[] newIndices = new String[count];
		System.arraycopy(s_indices, 0, newIndices, 0, count);
		
		return new Path(device, newSegments, newNames, newSpecs, newIndices, separators);
	}

	/* (non-Javadoc)
	 * @see st3.util.IPath#specialisations()
	 */
	@Override
	public String[] specialisations() {
		String[] specCopy = new String[s_specs.length];
		System.arraycopy(s_specs, 0, specCopy, 0, s_specs.length);
		return specCopy;
	}
	
	 private void writeObject(java.io.ObjectOutputStream out)
	 throws IOException {
		 out.writeObject(toPortableString());
	 }
	 
	 private void readObject(java.io.ObjectInputStream in)
	 throws IOException, ClassNotFoundException {
		Object p = in.readObject();
		if (!(p instanceof String)) throw new IOException("Expected String, got: " + p.getClass().getName());
		String path = (String)p;
		construct(path); // init this object
	 }
 }